...
**监督者模式**是一种[多代理](/oss/javascript/langchain/multi-agent)架构，其中中央监督者代理协调专门的辅助代理。当任务需要不同类型的专业知识时，这种方法表现出色。与其构建一个跨域管理工具选择的代理，不如创建由理解整体工作流的监督者协调的专注专家。
...
我们还将整合[人工审查](/oss/javascript/langchain/human-in-the-loop)，允许用户根据需要批准、编辑和拒绝操作（例如外发电子邮件）。

### 为什么使用监督者？

多代理架构允许您将[工具](/oss/javascript/langchain/tools)分配给不同的辅助代理，每个代理都有自己的提示或指令。考虑一个直接访问所有日历和电子邮件 API 的代理：它必须从许多相似的工具中选择，理解每个 API 的确切格式，并同时处理多个域。如果性能下降，将相关工具和关联提示分离到逻辑组中可能会有所帮助（部分是为了管理迭代改进）。
...
- [人工审查](/oss/javascript/langchain/human-in-the-loop)
...
整合敏感操作的[人工审查](/oss/javascript/langchain/human-in-the-loop)可能是明智的。LangChain 包含[内置中间件](/oss/javascript/langchain/human-in-the-loop#configuring-interrupts)来审查工具调用，在这种情况下是子代理调用的工具。

让我们为两个子代理添加人工审查：
- 我们将 `create_calendar_event` 和 `send_email` 工具配置为中断，允许所有[响应类型](/oss/javascript/langchain/human-in-the-loop)（`approve`、`edit`、`reject`）
- 我们**仅向顶级代理**添加[检查点保存器](/oss/javascript/langchain/short-term-memory)。这是暂停和恢复执行所必需的。
...
我们可以通过使用 [`Command`](https://langchain-ai.github.io/langgraphjs/reference/classes/langgraph.Command.html) 引用其 ID 来为每个中断指定决策。有关更多详细信息，请参阅[人工审查指南](/oss/javascript/langchain/human-in-the-loop)。为了演示目的，这里我们将接受日历事件，但编辑外发电子邮件的主题：
...
<Tip>
有关演示完整监督者模式（包括人工审查和高级信息流控制）的完整工作示例，请查看 LangChain.js 示例中的 [`supervisor_complete.ts`](https://github.com/langchain-ai/langchainjs/blob/main/examples/src/createAgent/supervisor_complete.ts)。
</Tip>
...
对于只有几个工具的简单情况，使用单个代理。当代理需要与用户对话时，请改用[交接](/oss/javascript/langchain/multi-agent#handoffs)。对于代理之间的点对点协作，请考虑其他多代理模式。
</Tip>

## 下一步

了解用于代理到代理对话的[交接](/oss/javascript/langchain/multi-agent#handoffs)，探索[上下文工程](/oss/javascript/langchain/context-engineering)以微调信息流，阅读[多代理概述](/oss/javascript/langchain/multi-agent)以比较不同模式，并使用 [LangSmith](https://smith.langchain.com) 调试和监控您的多代理系统。
