...
- 添加[重试](/oss/python/langchain/middleware/built-in#tool-retry)、[回退](/oss/python/langchain/middleware/built-in#model-fallback)和早期终止逻辑。
...
通过将它们传递给 [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 来添加中间件：
...
<CardGroup cols={2}>
    ...
    <Card title="测试代理" icon="scale-unbalanced" href="/oss/python/langchain/test">
        使用 LangSmith 测试您的代理。
    </Card>
</CardGroup>
